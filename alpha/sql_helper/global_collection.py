...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Alpha_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Alpha_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_item(keywoard):
    try:
        return (
            SESSION.query(Alpha_GlobalCollection.keywoard)
            .filter(Alpha_GlobalCollection.keywoard == keywoard)
            .count()
        )
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Alpha_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Alpha_GlobalCollection).all()
        for x in all_groups:
